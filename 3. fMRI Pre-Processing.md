## 1. Preparing fMRI Data for Pre-Processing 
...
3. `ls bold/028` double checks if all three files (f.nii, f.nii-infodump.dat, flf) are in the bold folder (there will be one folder for each scan is you are processing multiple scans simultaneously)
