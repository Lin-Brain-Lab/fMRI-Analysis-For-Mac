...
<details><summary>Details on Converting 1.dcm File to f.nii File</summary>
  
1. Add the 1.dcm file to corresponding epi data folder (Ex. epi1, epi2, epi3) in the following path: /Users/jessica/data_analysis/emoclips/s022/fmri_data
2. `tcsh`
3. `source .cshrc`
4. `setenv SUBJECTS_DIR /Users/jessica/data_analysis/emoclips/subjects`
5. `cd /Users/jessica/data_analysis/emoclips/s022/fmri_data/epi1`
6. `mri_convert 1.dcm f.nii` 
</details>

<details><summary>Details on Unpacking Dicom Files to Create f.nii Files (Old Version With Multiple DICOM Files)</summary>
...
% With EPI Data:
file_overlay_register='./bb_register_epi_032.dat';
...
9. In terminal `tcsh` then `source .cshrc` then `cd /Users/jessica/data_analysis/emoclips/s001/fmri_analysis` then `setenv SUBJECTS_DIR /Users/jessica/data_analysis/emoclips/subjects` the, `cd /Users/jessica/data_analysis/emoclips/s022/fmri_analysis` then create a script `vi s022_bb_register.script` with the contents of this script being as such, to allow for all runs. NOTE: press A key to edit, then ESC + :wq to save. 
```
bbregister --s s022 --mov mb_run_1_ref.mgh --init-reg bb_register_init_01.dat --reg bb_register_epi_01.dat --bold
bbregister --s s022 --mov mb_run_2_ref.mgh --init-reg bb_register_init_02.dat --reg bb_register_epi_02.dat --bold
bbregister --s s022 --mov mb_run_3_ref.mgh --init-reg bb_register_init_03.dat --reg bb_register_epi_03.dat --bold
```
10. `./ s022_bb_register.script` to run the the script (Note: if there is a permissions error at this point, try `chmod a+x s001_bb_register.script`). If creating a script in this manner does not work, you can run each line separately. If done successfully, there should be a bb_register_epi_01.dat in your fmri_analysis folder for each run.
...
1. Download [s5_read_erp_smsini_trigger_soa.m](https://github.com/Lin-Brain-Lab/fMRI-Analysis-For-Mac/blob/main/Scripts/Emo-Clips%20Analysis/2.%20EPI%20Co-Registration/s5_read_erp_smsini_trigger_soa.m) and add it to your subjects fmri_analysis folder. This script creates 3 .para files with the full MRI and stimuli trigger onset times. [s5_corrected_onset.m](https://github.com/Lin-Brain-Lab/fMRI-Analysis-For-Mac/blob/main/Scripts/Emo-Clips%20Analysis/3.%20GLM/s5_corrected_onset.m) will be more useful in that it will only print the onset times of the stimuli triggers, so that only the duration column needs to be added to use these .para files for the GLM analysis.
   
NOTE: Make sure that the arbritrary values given to the token are not also a value for one of your triggers (Ex. If the value of the sync token is 1e2 and you have a trigger denoted by the number 100, you should change the token to be 1.5e2 so it doesn't match the trigger number). 
   
2. You should then have a .para file for each run with the stimulus onset time in the first column, the trigger in the second, and the duration in the third. 

NOTE: For the individual vs. group-averaged valence analysis, you can open the .para files in excel and copy the correct timing to create the .para file needed for this analysis, with the stimulus onset time in the first column, the normalized rating binned in positive, neutral or negative in the second column, and the stimulus duration in the third column. The s5_corrected_onset.m script is good to ensure the timings are correct.
...
1. Follow steps on group level analysis from the [SMS-InI Analysis Page](https://github.com/Lin-Brain-Lab/fMRI-Analysis-For-Mac/blob/main/6.%20SMS-INI%20EEG-fMRI%20Data%20Analysis.md) and add to your emoclips/group_level/specific_hypothesis_individual folder and open in MatLab
...
2. Make a folder /Users/jessica/data_analysis/eegfmri/group_level/specific_hypothesis/average and go to this directory in MatLab
...
1. Download [g3_average_across_all_runs.m](https://github.com/Lin-Brain-Lab/fMRI-Analysis-For-Mac/blob/main/Scripts/Emo-Clips%20Analysis/4.%20Group-Level%20Analysis/g3_average_across_all_runs.m) and add to your emoclips/group_level/specific_hypothesis/all_runs folder and open in MatLab
2. Change the file_stc to the .stc files that were created in step 3.2 (ensure you are using the files with the correct hypotehsis number) and change the output_stem to your desired output name
3. If done successfully you should have one image that is the average of all three runs. For some reason this script outputs a error upon the first time running, just run it again and it should be fine
<details>
...
### 4.3 Output the FDR-Corrected Threshold for the Overall Activation Across All 3 Runs
1. Download [g4_average_surf_fdr_031324.m](https://github.com/Lin-Brain-Lab/fMRI-Analysis-For-Mac/blob/main/Scripts/Emo-Clips%20Analysis/4.%20Group-Level%20Analysis/g4_average_surf_fdr_031324.m) and add to your emoclips/group_level/specific_hypothesis/all_runs folder and open in MatLab
2. Change the cond_stem to the files name that you created in step 4.2
NOTE: Just use this script to get the FDR-corrected threshold, and the single-level contrasts (Group/Individual only for Signed/Unsigned)

### 4.4 Differential Contrast 
1. For the Individual vs. Group-Averaged contrast, use [g5_differential_contrast.m](https://github.com/Lin-Brain-Lab/fMRI-Analysis-For-Mac/blob/main/Scripts/Emo-Clips%20Analysis/4.%20Group-Level%20Analysis/g5_differential_contrast.m) and add it to emoclips/group_level/specific_hypothesis/differential folder, and use the .stc files you created in step 4.2 to compute the differential contrast. This script will output one map which is the average of all subjects. In the command window it will also output the minimum and maximum t-value which you can use to input the threshold (or make the threshold similar to the FDR-corrected one from step 4.3).
NOTE: This map is not FDR-corrected, lines 96-108 are commented out and calculate the FDR-corrected threshold, but since there are so little subjects, this is not always significant

### 4.5 Calculate P-Value for ROIs
1. Use [g6_pval_roi.m](https://github.com/Lin-Brain-Lab/fMRI-Analysis-For-Mac/blob/main/Scripts/Emo-Clips%20Analysis/4.%20Group-Level%20Analysis/g6_pval_roi.m) and add it to emoclips/group_level/specific_hypothesis/differential folder, and use the .stc files you created in step 4.2. This script outputs the P-value for each ROI and specifically highlights those that reach a significant (< 0.05) P-value.

# 5. fMRI Clustering
### 5.1 Cluster Brain Activity Based on Surface Values
1. Download [c1_clustering]() and add it to your emoclips/group_level/specific_hypothesis/all_runs folder and open in MatLab
2. Change line 3 to your 3 run average .stc file, the threshold should be the same as the previous group analysis [1 5] but you can edit this to have clearer clusters.
3. Click the "y" key to generate clustering results. Click "surf" on the GUI pop-up as we are using the brain activity overlay on the brain surface to generate clustering results. You will be prompted to save the clustering result save it as a "-lh.txt" file (make sure your folder path has no spaces in the folder names).
4. Click the "u" key to display the clustering results. Since no clustering file has been set you can click "Yes" to reset the clustering file. Now select the .txt file you have just created. The image should now have numbered clusters and the MatLab command window will contain the cluster information as such (you can click "u" to remove the numbers on the image):
```
labeling clusters from file [face_emo_clustering-lh.txt]...
cluster [001]: value=3.05 area=2698 (mm^2) x=-50.50 (mm) y=-44.00 (mm) z=2.60 (mm)  <<.92>>
cluster [002]: value=2.40 area=1658 (mm^2) x=-39.60 (mm) y=-63.60 (mm) z=38.10 (mm)  <<.91>>
cluster [003]: value=2.01 area=1232 (mm^2) x=-55.50 (mm) y=-0.00 (mm) z=27.00 (mm)  <<.84>>
cluster [004]: value=1.38 area=293 (mm^2) x=-37.40 (mm) y=39.70 (mm) z=-4.00 (mm)  <<.39>>
```
### 5.2 Cluster Brain Activity Based on Volume Values
1. Click the "y" key and select "vol" save the clustering result as a .txt file.
2. Click the "u" key and reset the clustering file then select the volume .txt clustering file you have just created.

### 5.3 Label Cluster Regions
After loading the cluster file, the command window will display the x,y, and z coordinates for each cluster. Use the [BioImage](https://bioimagesuiteweb.github.io/webapp/mni2tal.html) or [Neurosynth](https://neurosynth.org/locations/?x=-52&y=-32&z=28) atalas to find the region associated with theses coordinates.
