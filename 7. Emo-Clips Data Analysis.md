...
### 4.3 Average and Calculate the FDR-Corrected Threshold for the Overall Activation Across All 3 Runs
...
3. Change the output_stem to add the prefix Avg to the beginning of the file and use the same remainder of the name you labelled in cond_stem
4. If done successfully you should have one image that is the average of all three runs. This script also outputs the FDR-corrected threshold

### 4.4 Differential Contrast 
1. For the Individual vs. Group-Averaged contrast, use [g5_differential_contrast.m](https://github.com/Lin-Brain-Lab/fMRI-Analysis-For-Mac/blob/main/Scripts/Emo-Clips%20Analysis/4.%20Group-Level%20Analysis/g5_differential_contrast.m) and add it to emoclips/group_level/specific_hypothesis/differential folder. Copy the Avg_.stc files that you created in step 4.3 into the differential folder (these files populate in the emoclips/group_level/specific_hypothesis/all_runs folder).
