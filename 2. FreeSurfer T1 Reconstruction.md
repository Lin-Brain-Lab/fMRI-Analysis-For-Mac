...
16. `cd ../` make sure you are in orig file for next step (going back one folder) 
...
# FreeSurfer T1 Reconstruction For Mac Using 1.dcm File (Locally)
1. Create a folder in the following path: `/Users/jessica/data_analysis/emoclips/subjects/s0XX/mri/orig` and put your 1.dcm file in the orig folder
2. In the command window  `tcsh` 
2. `source .cshrc`
3. `mri_info` checks if all freesurfer items are there
6. `setenv SUBJECTS_DIR $PWD`
7. `setenv SUBJECTS_DIR /Users/jessica/data_analysis/seeg/subjects` set your subject directory
8. `echo $SUBJECTS_DIR` sees what path your subjects are in 
9. `cd /Users/jessica/data_analysis/seeg/subjects/s004/mri/orig` get into right directory in orig folder
10. `mri_info 1.dcm` to verify that the DICOM file can be read
11. `mri_convert 1.dcm 001.mgz` convert DICOM to .mgz file
12. `mri_info 001.mgz` verify that the .mgz file was created (you can also check that it is in the orig folder)
13. `freeview 001.mgz` to view the volume in freesurfer
14. `cd /Users/jessica/data_analysis/seeg/subjects/s0XX` change your directory back to the subjects folder
15.` recon-all -subjid s0XX -all` the reconscruction takes around 10 hours, a successful reconstruction with populate the folders label, mri, scripts, stats, surf, tmp, touch, and trash in the subjects folder
