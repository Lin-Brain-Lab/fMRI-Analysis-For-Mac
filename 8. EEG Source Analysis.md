...
3. Change line 3 to your subject ID `subject='180401_YTH';` and run. This step should take at least 1 hour.
...
2. Make sure line 7 `file_fwd='eeg_fwd_052524.mat';` is the same fwd file you created in the previous step
3. If done successfully, there should be two .stc files for lh and rh of each trigger (in this case 1, 10, 71, 99, 100, 199, 1000) in your subjects eeg_raw folder. The only trigger that has an event is 100.
4. Download [make_eeg_mne_052524.m](https://github.com/fahsuanlin/labmanual/blob/master/scripts/make_eeg_mne_052524.m) and add it to your eeg_raw folder
5.  Change lines 3-4 to ensure you are using the correct eeg_fwd and fwd_prep files
```
file_fwd='eeg_fwd_052524.mat';
file_bem='fwd_prep_041618.mat';
```
6. Change line 9 to your subject ID
7. Change all `erp_avg` to `erp` in lines 29-31
8. Make sure the number in 29-31 is the trigger with the event (in this case 100, which is the 5th trigger)
```
Y=erp{5}.erp; %
timeVec=erp{5}.timeVec; 
label=erp{5}.electrode_name;
```
...
3. Change line 10 and 11 to the .stc file with an event at the trigger
```
[stc,v_idx,a,b,timeVec]=inverse_read_stc('SSVEP_out_1_042124_dspm-lh.stc');
