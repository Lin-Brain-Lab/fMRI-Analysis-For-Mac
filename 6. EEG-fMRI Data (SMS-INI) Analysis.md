...
## Initialize Pathing and Environment 
...
### Calcuate Event-Rlated Potentials (ERPs)
...
6. Download [get_outside_erp.m](https://github.com/fahsuanlin/labmanual/blob/master/scripts/get_outside_erp.m) and add it to your eeg_analysis folder and run. This script gets ERP by averaging "epochs" of processed EEG waveform over trials.
7. Download [topology_31ch_default.mat](https://github.com/fahsuanlin/labmanual/blob/master/scripts/topology_31ch_default.mat) and add it to your eeg_analysis folder, then open the file and click the finish button.
8. Download [show_erp_scalp.m](https://github.com/fahsuanlin/labmanual/blob/master/scripts/show_erp_scalp.m) and add it to your eeg_analysis folder and run, there should be a pop-up. If you click the highlighted blue region on the scalp render, there should be a ERP plot pup-up.
...
9. Download [read_inside_eeg_042124.m](https://github.com/fahsuanlin/labmanual/blob/master/scripts/read_inside_eeg_042124.m) and add it to your eeg_analysis folder
10. Change from line 3 
...
```
11. To check results, use `etc_trace(cat(1,EEG_orig,ECG_orig),'fs',sfreq,'ch_names',label,'aux_data',{cat(1,EEG,ECG)},'trigger',TRIGGER);` there should be a ETC trace pop-up
12. Download [get_inside_erp.m](https://github.com/fahsuanlin/labmanual/blob/master/scripts/get_inside_erp.m) and add it to your eeg_analysis folder and run. This script gets ERP by averaging "epochs" of processed EEG waveform over trials.
13. Download [show_erp_scalp.m](https://github.com/fahsuanlin/labmanual/blob/master/scripts/show_erp_scalp.m) and add it to your eeg_analysis folder and run, there should be a pop-up
...
14. Download [read_smsini_eeg_042124.m](https://github.com/fahsuanlin/labmanual/blob/master/scripts/read_smsini_eeg_042124.m) and add it to your eeg_analysis folder and run
15. To check results, use `etc_trace(cat(1,EEG_orig,ECG_orig),'fs',sfreq,'ch_names',label,'aux_data',{cat(1,EEG,ECG)},'trigger',TRIGGER);` there should be a ETC trace pop-up
16. Download [get_smsini_erp.m](https://github.com/fahsuanlin/labmanual/blob/master/scripts/get_smsini_erp.m) and add it to your eeg_analysis folder and run. This script gets ERP by averaging "epochs" of processed EEG waveform over trials
17. Download [show_erp_scalp.m](https://github.com/fahsuanlin/labmanual/blob/master/scripts/show_erp_scalp.m) and add it to your eeg_analysis folder and run, there should be a pop-up
