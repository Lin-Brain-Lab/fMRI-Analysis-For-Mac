...
file_stc={ %input .stc files, since each run of emo-clips is a different legnth, they must be run one at a time
%        './s001_2_fsaverage_mb_run_1_acc';
%        './s001_2_fsaverage_mb_run_2_acc'; 
         './s001_2_fsaverage_mb_run_3_acc';
    };


hdr_length=30.0; %length of the haemodynamic response in seconds
hdr_pre=6.0; 	%length of the pre-stimulus time in relation to HDR in seconds 
TR=1.5; %length of time between the repition of sequences
...
erfmri_dur={ %created erfmri_dur to contain information about the duration of each clip for each run
   %'clipset_1_duration.txt';
   %'clipset_2_duration.txt';
    'clipset_3_duration.txt';
    };
...
file_mc_regressor=''; %motion correction coefficients for regession
...
for f_idx=1:length(file_stc)
    ...
    fprintf('<%s>...\r',fn);
    [stc_lh,v_lh,cc,dd]=inverse_read_stc(fn); %% start labelling here
...
for para_count=1:length(erfmri_para)
    ...
    [d_p,d_d]=textread(erfmri_dur{para_count},'%f %d'); % d_p is the triggers and d_d is the duration of the clip associated with that trigger
    para_time=[para_time; p_t+timepoints*TR*(para_count-1)];
    para_type=[para_type; p_p];
    p_d=zeros(size(p_p));
    for dur_count=1:length(d_d)
        dur_idx=find(p_p==d_p(dur_count));
        p_d(dur_idx)=d_d(dur_count);
    end;
...
for h=1:length(hypothesis)
    ...
    %cc_hdr(hypothesis{h}.rv)=hypothesis{h}.cvec; commented these out so it
    %runs now 
    ii=[];
