...
file_stc={
    % 's004_2_fsaverage_mb_run_1_acc';
    % 's004_2_fsaverage_mb_run_2_acc';
      's004_2_fsaverage_mb_run_3_acc';
    };
...
erfmri_para={
    % 'fahu_dur_fmri_soa_01.para';
    % 'fahu_dur_fmri_soa_02.para';
      'fahu_dur_fmri_soa_03.para';
};
...
glm_var={ %here we dont have the specific trigger code in the category because when making these .para files, the trigger code is not considered, rather it is assigned a number 1-8 that designates the trigger category
    [1]; %face 
    [2]; %object/ place
    [3]; %action
    [4]; %emotion
...
%hypotheses (in this case when doing run 2 for example which doesn't have
%face, comment out 1 in glm_var and for the hypothesis, remove the 1 so its
%[2 3 4 5 6 7 8] as these are the only categories it has)
hypothesis{1}.rv=[1 4];	
hypothesis{1}.cvec=[1 -1];


output_stem='sept24_run3_fmri_surf_soa_glm';
%------------------------------------------------------------------------------------
...
for glm_var_idx=1:length(glm_var) 
    ...
    if(~isempty(idx))
        ...
        soa_idx=round(para_time(idx)/TR*20); % for what x/TR*20 > size(scm), x/TR*20=7020 for a x (which is para_time) above 526.5s it gets cut off
        soa_idx(find(soa_idx<=0))=[];
        soa_idx(find(soa_idx>size(scm,1)))=[]; %soa is the timepoints that
        ...
        scm_tmp=scm_tmp(1:size(scm,1)); %resizes 
